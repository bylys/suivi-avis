...
async function generateImageWithChatGPT(prompt, cookies) {
    ...
    // Augmentation du timeout à 3 minutes (180000ms) pour éviter que Browserless ne coupe la connexion
    const browser = await chromium.connectOverCDP(`wss://chrome.browserless.io?token=${BROWSERLESS_TOKEN}&stealth&timeout=180000`);
    ...
    // Selecteur potentiel (à vérifier)
    const imageSelector = 'img[alt*="Generated by DALL·E"], img[alt*="DALL·E"]';
    
    try {
        await page.waitForSelector(imageSelector, { timeout: 120000 });
    } catch (e) {
        console.log("Le sélecteur d'image n'a pas été trouvé après 2 minutes.");
