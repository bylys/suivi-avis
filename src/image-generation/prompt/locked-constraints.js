...
function _appendLockedFinalConstraints(prompt, scene) {
  ...
  const _SVC_ACTION_LOCK = {
    ...
    'Étanchéité PVC': {
      action:    `PVC SINGLE-PLY MEMBRANE WATERPROOFING IN PROGRESS — FLAT ROOF.\nThe waterproofing system must be unmistakably PVC: a light grey or off-white synthetic single-ply sheet covering most of the flat roof surface, thermally welded at overlapping seams.\nEXACTLY TWO VISIBLE PROFESSIONAL WORKERS performing coordinated PVC welding:\nWorker 1 actively guides a professional electric hot-air welding gun (a compact industrial tool with a visible power cable — NO open flame, NO gas cylinder) along an overlapping PVC lap seam. The nozzle is pressed at the membrane overlap, emitting focused hot air to fuse the two plies.\nWorker 2 immediately follows with a small hand-held pressure roller, pressing and consolidating the freshly fused seam — the roller must be visibly in the worker's hand at the hot seam.\nBoth workers must be fully visible inside the frame, clearly separated, and simultaneously performing two distinct coordinated roles on the same active seam.\nThe PVC membrane — light grey or off-white — covers the majority of the flat roof surface. At least one thermowelded seam line is visible on the membrane.`,
      forbidden: `NO black EPDM rubber membrane as the dominant roofing surface.\nNO dark brown or black bituminous membrane roll.\nNO open flame, torch, or gas cylinder of any kind.\nNO cold adhesive spreading or bucket of cold glue as the primary bonding action.\nNO generic dark flat-roof coating applied with a roller or brush.\nNO single worker performing the welding task alone.\nNO pitched tiled roof surface.\nNO worker cropped out of frame or hidden behind the other worker.\nNO hot-air welding gun lying idle on the ground — it must be held and active in Worker 1 hand.\nNO pressure roller lying on the ground — it must be held by Worker 2 at the seam.\nNO plain white bâche or tarpaulin with no seam lines visible.\nNO overexposed white surface with no membrane texture, seam lines, or lap joints.`,
    },
  };
...
${isTreatmentService && hasWorkers && scene.state_level !== 'debut' && scene.state_level !== 'final' ? `
