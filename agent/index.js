    ...
    try {
        ...
        for (let taskIndex = 0; taskIndex < tasksToGenerate.length; taskIndex++) {
            ...
            if (lowerLabel.includes('vitrier') || lowerLabel.includes('vitrerie') || lowerLabel.includes('vitre') || lowerLabel.includes('vitrage') || lowerLabel.includes('fenêtre') || lowerLabel.includes('fenetre') || lowerLabel.includes('miroir') || lowerLabel.includes('miroiterie')) {
                ...
                negativeConstraint = "\n\n❌ INTERDICTION : AUCUN toit, AUCUN couvreur posant des tuiles, AUCUN élagage d'arbre, AUCUNE dépanneuse. UNIQUEMENT des maçons professionnels travaillant avec parpaings, béton, mortier, truelles, niveau à bulle, échafaudage de maçonnerie sécurisé ou au sol.";
                ...
                negativeConstraint = "\n\n❌ INTERDICTION : AUCUN toit, AUCUNE toiture, AUCUN élagage d'arbre, AUCUN couvreur, AUCUN maçon. UNIQUEMENT dépanneuse à plateau, technicien avec gilet haute visibilité jaune fluo, véhicule d'assistance routière ou dépannage de batterie sur bord de route sécurisé.";
                ...
                negativeConstraint = "\n\n❌ INTERDICTION : AUCUN toit, AUCUN couvreur posant des tuiles, AUCUN élagage d'arbre, AUCUN engin de terrassement lourd. UNIQUEMENT des professionnels du débarras/déménagement avec diables de manutention, cartons empilés, meubles protégés ou techniciens en tenue de protection blanche pour le syndrome de Diogène.";
                ...
                negativeConstraint = "\n\n❌ INTERDICTION : AUCUN toit, AUCUNE toiture, AUCUN élagage d'arbre, AUCUN nettoyeur haute pression sur toiture, AUCUNE dépanneuse. UNIQUEMENT des travaux de terrassement au sol, excavation, nivellement, tranchées VRD, assainissement, enrochement ou terrassement piscine.";
                ...
                negativeConstraint = "\n\n❌ INTERDICTION FORMELLE : AUCUNE échelle, AUCUN escabeau en appui contre la façade ou la gouttière (le travail sur échelle est strictement interdit par la sécurité du travail), AUCUN artisan seul en équilibre précaire en hauteur, AUCUN élagage d'arbre, AUCUN abattage, AUCUNE dépanneuse, AUCUN terrassement lourd, AUCUN travailleur debout sans protection sur tuiles glissantes. UNIQUEMENT intervention sécurisée depuis un échafaudage réglementaire avec garde-corps le long de la rive ou techniciens travaillant au sol (découpe et assemblage des profilés zinc/PVC sur établi au sol, pose de descentes d'eaux pluviales au sol). Artisans professionnels en duo équipés de casques de sécurité et gants.";
            }
            ...
            // Structure courte et positive (format recommandé directement par ChatGPT pour éviter tout faux mode édition)
            let safeTravaux = travauxLabel;
            if (lowerLabel.includes('abattage') || lowerLabel.includes('tronçonneuse') || lowerLabel.includes('tronconneuse') || lowerLabel.includes('dessouch')) {
                safeTravaux = "travaux d'élagage, entretien d'espaces verts et découpe professionnelle de bois au sol";
            } else if (lowerLabel.includes('démolition') || lowerLabel.includes('demolition')) {
                safeTravaux = "rénovation de maçonnerie et préparation du bâti";
            } else if (lowerLabel.includes('gouttière') || lowerLabel.includes('gouttiere') || lowerLabel.includes('chéneau') || lowerLabel.includes('cheneau') || lowerLabel.includes('descente')) {
                safeTravaux = "travaux sur gouttières depuis un échafaudage sécurisé avec garde-corps ou au sol (sans échelle)";
            } else if (lowerLabel.includes('couvreur') || lowerLabel.includes('toiture') || lowerLabel.includes('couverture')) {
                safeTravaux = "travaux de couverture depuis un échafaudage de sécurité ou au sol (sans échelle)";
            }

            const secureRichPrompt = `Génère une photo smartphone ultra-réaliste à ${locationStr} montrant exactement ${nbOuvriers} : travaux de ${safeTravaux}, artisans actifs équipés de leurs outils et protections professionnelles (EPI), ambiance ${lumiere}, vue ${pointDeVue}, format ${orientation}.`;
