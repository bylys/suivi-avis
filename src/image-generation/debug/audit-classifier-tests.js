...
export async function runAuditClassifierTests() {
  ...
  test('AUD-SUM2', 'summary: STATE_LOCKED=47, DEFERRED=8, ROUTED_TO_SPECIFIC_SCENE=110', () => {
    const { summary } = generateServiceCoverageAudit();
    assert(summary.STATE_LOCKED === 47,
      `Expected STATE_LOCKED=47, got ${summary.STATE_LOCKED}`);
    assert(summary.DEFERRED === 8,
      `Expected DEFERRED=8, got ${summary.DEFERRED}`);
    assert(summary.ROUTED_TO_SPECIFIC_SCENE === 110,
      `Expected ROUTED_TO_SPECIFIC_SCENE=110, got ${summary.ROUTED_TO_SPECIFIC_SCENE}`);
