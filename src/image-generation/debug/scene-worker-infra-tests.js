...
export async function runSceneWorkerInfraTests() {
  ...
  test('SWI-EX1', 'scene_reset_exclude absent → pre-existing exclusions preserved', () => {
    ...
    assert(Array.isArray(result.exclude), 'exclude must remain an array');
    assert(result.exclude.includes('sentinel-value'),
      `sentinel-value was wiped — exclude after: ${JSON.stringify(result.exclude)}`);
  });

  // ─── SWI-EX2: scene_reset_exclude code path present in service-resolver ──
      ...
      scenarios: [
        {
          _for: '^test.*service$',
          _state_for: 'encours',
          _visual_family: 'SWI-PL-STATE-LOCKED',
          _access_configuration: 'SWI_PL_LOCKED',
        },
        {
          _for: '^test.*service$',
          _visual_family: 'SWI-PL-GENERIC',
          _access_configuration: 'SWI_PL_GENERIC',
        },
      ],
    };
